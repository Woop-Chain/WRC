...
Initialization
********************************/
const WOC = 1000000000000000000 // 1 WOC in atto
...
********************************/
//e.g.
// http://localhost:3000/fund?address=woc103q7qe5t2505lypvltkqtddaef5tzfxwsse4z7// -> 0x7c41e0668b551f4f902cfaec05b5bdca68b124ce
app.get('/fund', async (req, res) => {
	...
	const faucetAddress = hexToWocAddress(wiki, getContractAddress(FaucetJSON))
	let faucetBalance = (await wiki.blockchain.getBalance({ address:faucetAddress }).catch((error) => {
		res.send({success: false, message: "Could not connect to Woop"})
		err = true
	})).result
	faucetBalance = new wiki.utils.Unit(faucetBalance).asWei().toEther()
	console.log('balance', faucetBalance)
	if(faucetBalance < (queue.length+1)*(txRate/WOC)) {
		res.send({
			success: false,
...
setInterval(async () => {
	...
	let accountBalance1 = (await wiki.blockchain.getBalance({ address:front.address }).catch((error) => {
		res.send({success: false, message: "Could not connect to Woop"})
	})).result
	accountBalance1 = new wiki.utils.Unit(accountBalance1).asWei().toEther()

	const faucet = getContractInstance(wiki, FaucetJSON)
	...
	try {
		...
		let accountBalance2 = (await wiki.blockchain.getBalance({ address:front.address }).catch((error) => {
			res.send({success: false, message: "Could not connect to Woop"})
		})).result
		accountBalance2 = new wiki.utils.Unit(accountBalance2).asWei().toEther()
...
// 	const faucet = hexToWocAddress(wiki, getContractAddress(FaucetJSON))
