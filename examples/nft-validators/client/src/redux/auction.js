...
import WRC721Auction from '../build/contracts/WRC721Auction.json'
// import { approveWRC20 } from './wrc20'
import { updateDialogState, updateProcessing, getBalances } from './woop'
...
export const toggleAuction = ({ value }) => async (dispatch, getState) => {
    ...
    const { wiki, contract, active } = await getContract(getState().woopReducer, WRC721Auction)
...
export const getItem = (i, dispatchItem = false) => async (dispatch, getState) => {
    ...
    if (item.bids.length !== totalBids) {
        for (let j = item.bids.length; j < totalBids; j++) {
            ...
            amount = new wiki.utils.Unit(amount).asWei().toEther().toString()
    ...
    if (!item.limit) {
        ...
        item.price = new wiki.utils.Unit(item.price).asWei().toEther().toString()
...
export const getInventory = () => async (dispatch, getState) => {
    ...
    //get
    let funds = (await wiki.blockchain.getBalance({ address: contract.address }).catch((err) => {
        console.log(err);
    }))
    if (funds) {
        funds = new wiki.utils.Unit(funds.result).asWei().toEther().toString()
    }
    //get state
