...
const WRC721Auction = artifacts.require("WRC721Auction");
...
contract("WRC721", (accounts) => {
	let wrc721, auction
	const woc = 	bn('1000000000000000000');
	const wocTenth = 	bn('100000000000000000');
	const price = 	bn('100000000000000000'); // woc tenth of WOC
	...
	it("should be deployed", async () => {
		...
		auction = await WRC721Auction.deployed()
		assert.ok(wrc721 && auction)
	})
	...
	********************************/
	...
	it("should allow somewoc to name their address", async () => {
	...
	it("should let bob out bid alice", async () => {
		...
		await auction.makeBid(1, {
			from: bob,
			value: price.add(wocTenth).add(wocTenth),
			gasLimit,
			gasPrice
		}).catch((e) => {
			// console.log(e) //capture the exception here
		...
		assert.deepEqual(balanceAfter.sub(balanceBefore), price.add(wocTenth)) 
	})
	...
	********************************/
	...
	it("should allow the minter / owner to close the action and distribute NFTs", async () => {
		...
		//check alice got her bid amount back
		const tokens = (await wrc721.balanceOf(bob)).toNumber()
		for (let i = 0; i < tokens; i++) {
			const tokenId = (await wrc721.tokenOfOwnerByIndex(bob, i)).toNumber()
