...
const { initWoop, transfer, toWocAddress } = require('./woop')
...
app.get('/exposeAddress', async (req, res) => {
	...
	const faucetAddress = wiki.wallet.signer.bech32Address
	res.send({
...
app.post('/fund', express.json(), async (req, res) => {
	...
	try {
		address = toWocAddress(wiki, address)
	} catch (error) {
		res.send({
			success: false,
			message: `Invalid WOC address ${address}`
		})
		console.error(error)
		return
	}
	...
	let faucetBalance = await wiki.wallet.signer.getBalance()
	faucetBalance = faucetBalance.balance
	console.log('balance', faucetBalance)
	if(faucetBalance < (queue.length+1)*(parseInt(txRate) * WOC)) {
		res.send({
			success: false,
